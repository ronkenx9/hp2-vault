...
export const hyperspeedPresets = {
    // Preset "two" from React Bits — red + lavender, mountain distortion
    two: {
        onSpeedUp: () => { },
        onSlowDown: () => { },
...
export const HP2_PRESET: any = hyperspeedPresets.two;

const DEFAULT_EFFECT_OPTIONS: any = {
    onSpeedUp: () => { },
    onSlowDown: () => { },
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: any }) => {
    ...
    `;
        ...
        /* ───── App Class ───── */
        class App {
            options: any; container: HTMLDivElement; renderer: THREE.WebGLRenderer; composer: EffectComposer;
            ...
            disposed: boolean; hasValidSize: boolean; renderPass: any; bloomPass: any;
            ...
            onMouseDown() { if (this.options.onSpeedUp) this.options.onSpeedUp(); this.fovTarget = this.options.fovSpeedUp; this.speedUpTarget = this.options.speedUp; }
            onMouseUp() { if (this.options.onSlowDown) this.options.onSlowDown(); this.fovTarget = this.options.fov; this.speedUpTarget = 0; }
            ...
            dispose() {
                this.disposed = true;
                this.scene.traverse((obj: any) => { if (obj.isMesh) { obj.geometry?.dispose(); if (Array.isArray(obj.material)) obj.material.forEach((m: any) => m.dispose()); else obj.material?.dispose(); } });
                this.scene.clear();
        ...
        options.distortion = distortions[options.distortion] || distortions.mountainDistortion;
    ...
    return <div id="lights" ref={hyperspeed} className="w-full h-full absolute inset-0 overflow-hidden"></div>;
};
